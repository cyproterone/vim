...
local keymap = function (buf)
  ...
  map("<S-Tab>", "defx#async_action('open_tree', ['toggle', 'recursive'])",       opts)
  map("<Enter>", "defx#async_action('drop')",                                     opts)
  map("`",       "defx#async_action('drop') . ':Defx <CR>'",                      opts)
  map("<Up>",    "line('.') == 1 ? 'G' : '<Up>'",                                 opts)
  map("<Down>",  "line('.') == line('$') ? 'gg' : '<Down>'",                      opts)
  map("<Right>", "defx#async_action('drop')",                                     opts)
  map("<Left>",  "defx#async_action('cd', ['..'])",                               opts)
  ...
  map("x",       "defx#async_action('execute_system')",                           opts)
